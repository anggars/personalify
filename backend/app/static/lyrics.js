...
async function searchArtist() {
    ...
    try {
        const res = await fetch(`/genius/search_artist?q=${encodeURIComponent(query)}`);
...
async function getArtistSongs(artistId) {
    ...
    try {
        const res = await fetch(`/genius/artist_songs?artist_id=${artistId}`);
        if (!res.ok) {
...
async function getSongLyrics(songId) {
    ...
    try {
        const res = await fetch(`/genius/song_lyrics?song_id=${songId}`);
